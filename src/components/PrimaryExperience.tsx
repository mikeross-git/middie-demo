...
function ProfileDetail({ profile, onBack }: { profile: MatchProfile; onBack: () => void }) {
  ...
  return <div className="primary-experience profile-detail"><ExperienceHeader /><div className="profile-detail__intro"><p>● NEW</p><h1>{profile.name} wants<br />to meet you.</h1><span className="red-rule" /><div>{profile.name} liked your video and wants to meet up. Check out her IG to get to know her better.</div></div><section className="profile-panel"><Portrait profile={profile} className="profile-panel__portrait" /><h2>{profile.name}, {profile.age}</h2><p className="profile-panel__location">{profile.location}</p><div className="profile-facts"><span>♙<small>{profile.occupation}</small></span><span>◇<small>{profile.school}</small></span><span>⌖<small>{profile.location}</small></span></div><div className="instagram-block"><span>Her Instagram <i>✓</i></span><strong>{profile.instagram}</strong></div><div className="lifestyle-gallery">{profile.gallery.map((item, index) => <div className={`lifestyle-tile lifestyle-tile--${item}`} key={`${item}-${index}`}><span>{index + 1}</span></div>)}</div><PrimaryButton onClick={() => setMessage(true)}>SEND A MESSAGE</PrimaryButton><SecondaryButton onClick={onBack}>KEEP BROWSING</SecondaryButton></section>{message && <div className="prototype-toast prototype-toast--message" role="dialog"><button onClick={() => setMessage(false)} aria-label="Close">×</button><strong>Connection preview</strong><p>In the real app, this would open your preferred way to connect.</p></div>}</div>
}
...
function EmptyMatches({ onHowItWorks }: { onHowItWorks: () => void }) {
  ...
  return <div className="primary-experience empty-matches"><ExperienceHeader /><ExperienceTitle title="Your Matches"><h2>No new matches this week.</h2><p>Good things take a minute. We’re still sharing your profile with thoughtful women who want something real.</p><p>Check back next Sunday for new matches.</p></ExperienceTitle><div className="empty-matches__scene" aria-hidden="true"><span className="empty-matches__glow" /><span className="empty-matches__envelope">♡</span><span className="empty-matches__chair" /></div><div className="empty-matches__actions"><PrimaryButton onClick={() => setActive(true)}>{active ? 'PROFILE ACTIVE' : 'KEEP PROFILE ACTIVE'}</PrimaryButton><button onClick={onHowItWorks}>HOW IT WORKS</button></div><p className="empty-matches__sunday"><span>♡</span>New matches every Sunday.</p>{active && <div className="prototype-toast" role="status">Your profile will stay active for next Sunday’s introductions.</div>}</div>
}
