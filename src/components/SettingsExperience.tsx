...
function NewVideoSettings({ back }: { back: () => void }) {
  ...
  return <div className="settings-screen"><SettingsHeader onBack={back} /><SettingsTitle title="Record a New Video" copy="Watch your current intro, then choose whether to replace it." /><div className="current-video"><span>{state === 'recording' ? `REC · 00:0${seconds}` : state === 'ready' ? 'New intro ready' : 'Current Intro · 0:27'}</span><div className={`current-video__portrait current-video__portrait--${state}`}><i /><b /><em>{state === 'recording' ? '●' : state === 'ready' ? '✓' : '▶'}</em></div></div><p className="replace-note">Recording a new video will overwrite<br />your current intro.</p><div className="video-settings-actions"><div className="settings-info">ⓘ<span>You can record up to 30 seconds.</span></div>{state === 'ready' ? <><PrimaryButton onClick={() => setState('current')}>Use New Video</PrimaryButton><SecondaryButton onClick={record}>Record Again</SecondaryButton></> : <><SecondaryButton onClick={back}>Keep Current Video</SecondaryButton><PrimaryButton disabled={state === 'recording'} onClick={record}>{state === 'recording' ? 'Recording…' : 'Record New Video'}</PrimaryButton></>}</div></div>
}
